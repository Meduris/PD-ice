	...
	public WndSettings(boolean inGame) {
		...
		if (!inGame) {
			...
			RedButton btnOrientation = new RedButton(orientationText()) {
				@Override
				protected void onClick() {
					ShatteredPixelDungeon.counterLandscape(false);
			...
			CheckBox btnCounterLandscape = new CheckBox(TXT_SWITCH_COUNTER_LAND) {
				...
				protected void onClick() {
					super.onClick();
					ShatteredPixelDungeon.counterLandscape(checked());
				}
			};
			btnCounterLandscape.setRect(0, btnOrientation.bottom() + GAP,
					WIDTH, BTN_HEIGHT);
			btnCounterLandscape.checked(ShatteredPixelDungeon
					.counterLandscape());
			btnCounterLandscape.enable(android.os.Build.VERSION.SDK_INT >= 9
